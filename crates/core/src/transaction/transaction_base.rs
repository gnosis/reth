...
impl TransactionBase {
  ...
  fn decode(d: &Rlp) -> Result<Transaction, DecoderError> {
    if d.item_count()? != 9 {
      return Err(DecoderError::RlpIncorrectListLen);
    }
    let hash = keccak(d.as_raw());
    let data = Data::Legacy(Self::decode_data(d, 0)?);
    let mixed_v: SigVLegacy = d.val_at(6)?;
    let r = d.val_at(7)?;
    let s = d.val_at(8)?;
  ...
  fn decode_data(d: &Rlp, offset: usize) -> Result<TransactionBase, DecoderError> {
    Ok(TransactionBase {
